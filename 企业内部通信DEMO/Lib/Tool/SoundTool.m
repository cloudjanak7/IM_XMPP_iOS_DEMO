...
- (void)playSoundWithName:(NSString *)name
{
    ...
    // 从字典中取出SystemSoundID
    SystemSoundID soundID = [_soundsDict[name] unsignedIntValue];

    // 判断文件是否存在
    NSAssert(soundID > 0, @"%@ %ud - 音效文件不存在！", name, (unsigned int)soundID);
...
+ (AVAudioPlayer *)audioPlayerWithData:(NSData *)data
{
    ...
    AVAudioPlayer *player = [[AVAudioPlayer alloc] initWithData:data fileTypeHint:@"caf" error:&error];
